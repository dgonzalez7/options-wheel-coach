...
The dashboard reads its state from `dashboard/data.js`, which is generated by:

```bash
python scripts/build_dashboard_data.py
```

That script reads `data/portfolio.json` and `data/positions_open.json`, validates them through the pydantic schemas, and emits a small JS file that assigns the full payload to `window.dashboardData`. Because the dashboard loads via a `<script>` tag rather than `fetch()`, it works directly from a `file://` URL — double-click `dashboard/index.html` and it opens. No server, no firewall prompts.

Run the build script whenever data changes manually. Phase 2's daily updater calls it as its final step, so once that lands the dashboard always reflects the latest snapshot automatically.

`scripts/serve.py` (which serves the workspace folder on `http://localhost:8000/`) is still included for development scenarios where serving HTTP is preferred — for example, testing changes that involve real network requests.
